...
def planck_with_mod(wav, T, N, vmax, vphot, tau=0.55, occult = 1, ve=0.32,
                    ...
                    vref=0., t0=120960, lam=10506.3653, **pars): #blackbody
    #a = 2.0*h*pi*c**2
    #b = h*c/(wav*k*T)
    ...
    #gau1 = gaussian(wav[(wav>cen1-5*sig1) & ((wav<cen1+5*sig1))], 1e-17*amp1, cen1, sig1)
    #gau2 = gaussian(wav[(wav>cen2-5*sig2) & ((wav<cen2+5*sig2))], 1e-17*amp2, cen2, sig2)
    
    #final = N*intensity*correction
    #final[(wav>cen1-5*sig1) & ((wav<cen1+5*sig1))] += gau1
    #final[(wav>cen2-5*sig2) & ((wav<cen2+5*sig2))] += gau2
    
    return N*intensity*correction+gau1+gau2 #final
